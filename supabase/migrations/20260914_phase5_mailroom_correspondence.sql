...
-- Private browser-demo access is limited to Mailroom document paths only.
-- Path format: demo/<tenant-id>/mailroom/<mailroom-item-id>/<object-name>
...
begin
  ...
  if not exists (
    ...
    where t.id = v_item.tenant_id
      and t.name = 'Therassistant Demo'
      and coalesce((t.settings ->> 'demo')::boolean, false) is true
  ) then
    raise exception 'Mailroom transition is limited to the Therassistant Demo tenant.' using errcode = '42501';
  end if;
  ...
  case p_action
    when 'review' then
      if v_old_status <> 'new' then
        raise exception 'Review is only allowed from new.' using errcode = '22023';
      end if;
      v_new_status := 'reviewed';

    when 'require_action' then
      if v_old_status not in ('new', 'reviewed') then
        raise exception 'Action Required is only allowed from new or reviewed.' using errcode = '22023';
      end if;
      v_new_status := 'action_required';

    when 'start' then
      if v_old_status not in ('action_required', 'pending') then
        raise exception 'Start is only allowed from action required or pending.' using errcode = '22023';
      end if;
      v_new_status := 'in_progress';

    when 'pend' then
      if v_old_status not in ('action_required', 'in_progress') then
        raise exception 'Pend is only allowed from action required or in progress.' using errcode = '22023';
      end if;
      if nullif(trim(coalesce(p_reason, '')), '') is null then
        raise exception 'A pending reason is required.' using errcode = '22023';
      end if;
      v_new_status := 'pending';

    when 'resolve' then
      if v_old_status not in ('action_required', 'in_progress', 'pending') then
        raise exception 'Resolve is only allowed from action required, in progress, or pending.' using errcode = '22023';
      end if;
      v_new_status := 'resolved';

    when 'close' then
      if v_old_status not in ('new', 'reviewed', 'resolved') then
        raise exception 'Close is only allowed from new, reviewed, or resolved.' using errcode = '22023';
      end if;
      if exists (
        select 1
    ...
    when 'reopen' then
      if v_old_status not in ('resolved', 'closed') then
        raise exception 'Reopen is only allowed from resolved or closed.' using errcode = '22023';
      end if;
      v_new_status := 'action_required';
  ...
  -- Locate current active correspondence work, if any.
  select *
  into v_work
  from public.workqueue_items w
  where w.tenant_id = v_item.tenant_id
  ...
  if p_action = 'require_action' then
    ...
    if v_work.id is null then
      raise exception 'Active correspondence work item not found.' using errcode = 'P0002';
    end if;
    v_old_work_status := v_work.workqueue_status;
    update public.workqueue_items
    set workqueue_status = 'in_progress',
        updated_at = v_now
    where id = v_work.id
    returning * into v_work;
    ...
    if v_old_work_status is distinct from v_work.workqueue_status then
      insert into public.workqueue_history (
        tenant_id, workqueue_item_id, old_status, new_status,
        old_priority, new_priority, changed_by, note
      ) values (
        v_item.tenant_id, v_work.id, v_old_work_status, v_work.workqueue_status,
        v_work.priority, v_work.priority, auth.uid(), coalesce(nullif(trim(p_reason), ''), 'Correspondence work started.')
      );
    end if;
    ...
    if v_old_work_status is distinct from v_work.workqueue_status then
      insert into public.workqueue_history (
        ...
        v_item.tenant_id, v_work.id, v_old_work_status, v_work.workqueue_status,
        v_work.priority, v_work.priority, auth.uid(), trim(p_reason)
      );
    end if;
    ...
    insert into public.workqueue_history (
      ...
      v_item.tenant_id, v_work.id, v_old_work_status, v_work.workqueue_status,
      v_work.priority, v_work.priority, auth.uid(), coalesce(nullif(trim(p_reason), ''), 'Correspondence resolved.')
    );
    ...
    if v_work.id is null then
      ...
      if v_work.id is not null then
        ...
        set workqueue_status = 'reopened',
            ...
            assigned_user_id = v_item.assigned_user_id,
            completed_at = null,
            completed_by = null,
            updated_at = v_now
        where id = v_work.id
        ...
        insert into public.workqueue_history (
          ...
          v_item.tenant_id, v_work.id, v_old_work_status, v_work.workqueue_status,
          v_work.priority, v_work.priority, auth.uid(), coalesce(nullif(trim(p_reason), ''), 'Correspondence reopened.')
        );
      else
        insert into public.workqueue_items (
          tenant_id,
  ...
  set status = v_new_status,
      reviewed_at = case
        when p_action = 'review' then coalesce(reviewed_at, v_now)
        else reviewed_at
      end,
      closed_at = case
        when p_action = 'close' then v_now
