...
begin
  ...
  -- 14-day payer follow-up after submission.
  ...
  where ca.tenant_id = p_tenant_id
    ...
    and not exists (
      ...
      where w.tenant_id = ca.tenant_id
        ...
        and w.title = 'Credentialing: Payer Follow-Up'
    );

  get diagnostics v_count = row_count;
  v_created := v_created + v_count;

  -- Payer has requested more information: surface urgent work immediately.
  insert into public.workqueue_items (
    tenant_id,
    workqueue_type,
    workqueue_status,
  ...
  where ca.tenant_id = p_tenant_id
    ...
    and not exists (
      ...
      where w.tenant_id = ca.tenant_id
        ...
        and w.source_object_id = ca.id
        and w.workqueue_type = 'credentialing_followup'
        and w.title = 'Credentialing: Additional Information Requested'
    );
  ...
  where ca.tenant_id = p_tenant_id
    ...
    and not exists (
      ...
      where w.tenant_id = ca.tenant_id
        ...
        and w.source_object_id = ca.id
        and w.workqueue_type = 'credentialing_followup'
        and w.title = 'Credentialing: Approval Details Needed'
    );
  ...
  with candidates as (
    select
      ...
      trim(concat_ws(' ', p.first_name, p.last_name)) as provider_name,
