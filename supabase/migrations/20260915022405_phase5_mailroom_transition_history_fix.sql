...
begin
  ...
  if p_action = 'require_action' then
    if v_work.id is null then insert into public.workqueue_items (tenant_id, workqueue_type, workqueue_status, priority, source_object_type, source_object_id, title, description, due_date, assigned_user_id, created_by) values (v_item.tenant_id, 'correspondence', 'open', v_priority, 'mailroom_item', v_item.id, v_item.subject, replace(v_item.correspondence_type, '_', ' ') || ' correspondence requires follow-up.', v_item.due_date, v_item.assigned_user_id, auth.uid()) returning * into v_work; end if;
    ...
    if v_work.id is null then
      ...
      if v_work.id is not null then
        ...
        insert into public.workqueue_items (tenant_id, workqueue_type, workqueue_status, priority, source_object_type, source_object_id, title, description, due_date, assigned_user_id, created_by) values (v_item.tenant_id, 'correspondence', 'open', v_priority, 'mailroom_item', v_item.id, v_item.subject, replace(v_item.correspondence_type, '_', ' ') || ' correspondence requires follow-up.', v_item.due_date, v_item.assigned_user_id, auth.uid()) returning * into v_work;
      end if;
    end if;
  end if;
