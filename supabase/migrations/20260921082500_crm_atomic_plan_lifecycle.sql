...
begin
  ...
  if exists (
    ...
    where account_id = p_account_id and status in ('draft','active','defaulted')
  ) then
  ...
  values (
    ...
    'not_generated',0,p_actor_email,p_actor_email
  )
  ...
  select v_plan_id,(item->>'sequence_number')::integer,(item->>'due_date')::date,
         (item->>'amount_due_cents')::integer,0,'upcoming'
  from jsonb_array_elements(p_installments) as item;

  select to_jsonb(p) || jsonb_build_object('schedule',p_installments)
  into v_snapshot from public.crm_payment_plans p where p.id=v_plan_id;

  insert into public.crm_payment_plan_versions (plan_id,version_number,snapshot,reason,created_by)
...
declare
  ...
  v_snapshot jsonb;
begin
  select * into v_plan from public.crm_payment_plans where id=p_plan_id for update;
  ...
  where plan_id=p_plan_id and amount_paid_cents=0;

  insert into public.crm_installments (
    plan_id,sequence_number,due_date,amount_due_cents,amount_paid_cents,status
  )
  select p_plan_id,(item->>'sequence_number')::integer,(item->>'due_date')::date,
         (item->>'amount_due_cents')::integer,0,'upcoming'
  from jsonb_array_elements(p_installments) as item;
  ...
  set status='draft',
      ...
      updated_by=p_actor_email,updated_at=now()
  where id=p_plan_id
  returning * into v_updated;

  select coalesce(max(version_number),0)+1 into v_version
  from public.crm_payment_plan_versions where plan_id=p_plan_id;
  ...
  values (
    v_plan.account_id,'payment_plan_modified',
    'Payment plan terms modified; a new signed agreement is required.',
    'crm_payment_plans',p_plan_id,jsonb_build_object('version',v_version),p_actor_email
  );
