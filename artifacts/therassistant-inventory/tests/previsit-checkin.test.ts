...
test("provider patient review prioritizes submitted pre-visit visit questions", async () => {
  ...
  const result = buildPatientReviewCheckIn({
    ...
    responses: {
      pre_visit: {
        submitted_at: "2026-09-19T20:00:00.000Z",
        visit_questions: {
  ...
  assert.equal(result.hasSubmittedPreVisit, true);
});

test("provider patient review ignores incomplete pre-visit drafts", async () => {
  const { buildPatientReviewCheckIn } = await import("../src/domains/scheduling/patient-review-model.ts");
  const result = buildPatientReviewCheckIn({
    focus_today: "Legacy focus",
    mood: "Legacy mood",
...
test("provider patient review maps submitted safety responses", async () => {
  const { buildPatientReviewCheckIn } = await import("../src/domains/scheduling/patient-review-model.ts");
