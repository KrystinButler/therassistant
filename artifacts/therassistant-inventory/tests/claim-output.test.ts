...
test("837P export uses configured trading-partner and claim data without demo placeholders", () => {
  ...
  assert.equal(filingIndicatorFrom837P(x12), "CI");
...
test("837P export uses the configured Medicaid filing indicator instead of hard-coded commercial insurance", () => {
  ...
  assert.equal(filingIndicatorFrom837P(x12), "MC");
  assert.notEqual(filingIndicatorFrom837P(x12), "CI");
});
