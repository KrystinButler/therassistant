...
export function JournalPanel({ chart, onChanged }: { chart: PatientChart; onChanged: () => Promise<void> }) {
  ...
  return <section className="thera-card">
    <div className="thera-card-header"><div><h2>Patient Journal</h2><p>Patient-reported reflections, including those transcribed by staff, remain separate from signed clinical documentation unless a clinician deliberately incorporates relevant information.</p></div></div>
    ...
    <div className="thera-form-grid" style={{ marginBottom: 18 }}>
      ...
      <label className="thera-field thera-span-2"><span className="thera-field-label">Patient-reported entry (recorded by staff)</span><textarea className="thera-input" rows={4} value={entryText} onChange={(e) => setEntryText(e.target.value)} /></label>
