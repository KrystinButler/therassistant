...
export function PostPaymentDrawer({ open, onOpenChange, data, saving, onSave }: { open: boolean; onOpenChange: (open: boolean) => void; data: Data; saving: boolean; onSave: (source: "insurance" | "patient", form: PaymentForm) => Promise<void> | void }) {
  ...
  return <WorkDrawer open={open} onOpenChange={onOpenChange} dirty={dirty} title="Post Payment" subtitle="Record and allocate a payment without leaving the payment workspace." footer={footer}>
    <div className="thera-form-grid">
      ...
      <label>Claim allocation<select className="thera-input" value={form.claimId} onChange={(e) => setForm((v) => ({ ...v, claimId: e.target.value }))}><option value="">Unapplied payment</option>{data.claims.filter((row) => !["voided", "reversed", "paid"].includes(String(row.claim_status))).map((row) => <option key={row.id} value={row.id}>{String(row.patient_control_number ?? "Claim")} · {row.clientName}</option>)}</select></label>
      <label>Allocation amount<input className="thera-input" type="number" min="0" step="0.01" value={form.allocation} placeholder={form.claimId ? form.amount || "0.00" : "Unapplied"} disabled={!form.claimId} onChange={(e) => setForm((v) => ({ ...v, allocation: e.target.value }))} /></label>
...
export function PaymentDetailDrawer({ open, onOpenChange, row, data, onReverse }: { open: boolean; onOpenChange: (open: boolean) => void; row: PaymentRow | null; data: Data; onReverse: (row: PaymentRow) => void }) {
  ...
  const footer = <div className="thera-filter-row" style={{ justifyContent: "space-between" }}><button type="button" className="thera-action secondary" onClick={() => onOpenChange(false)}>Close</button>{!["reversed", "voided"].includes(String(row.payment_status)) && <button type="button" className="thera-action secondary" onClick={() => onReverse(row)}>Reverse Payment</button>}</div>;
  return <WorkDrawer open={open} onOpenChange={onOpenChange} title={`${money(Number(row.amount_cents ?? 0))} payment`} subtitle={`${row.clientName} · ${row.payerName}`} badges={<StatusBadge value={String(row.payment_status)} />} footer={footer}>
    ...
    <section className="thera-card"><h2>Claim allocations</h2>{allocations.length ? allocations.map((item) => <div key={item.id} className="thera-card"><strong>{item.claimControlNumber}</strong><div>{item.patientName} · {money(Number(item.amount_cents ?? 0))}</div>{item.reversed_at && <div className="thera-table-subtext">Reversed {dateTime(String(item.reversed_at))}</div>}</div>) : <div className="thera-empty">This is currently an unapplied payment.</div>}</section>
