...
function formFrom(record?: Record<string, unknown>): DrawerForm { return { client_id: text(record?.client_id, ""), payer_id: text(record?.payer_id, ""), rendering_provider_id: text(record?.rendering_provider_id, ""), billing_provider_id: text(record?.billing_provider_id, ""), patient_control_number: text(record?.patient_control_number, ""), payer_claim_number: text(record?.payer_claim_number, ""), service_date_from: text(record?.service_date_from, "").slice(0, 10), service_date_to: text(record?.service_date_to, "").slice(0, 10), total_charge_cents: Number(record?.total_charge_cents ?? 0) }; }
...
export function ClaimWorkDrawer({ claim, open, onOpenChange, mode = "edit", queuePosition, onPrevious, onNext, previousDisabled, nextDisabled }: Props) {
  ...
  function correct(target: ClaimCorrectionTarget) { if (target === "subscriber") { if (form.client_id) navigate(`/clients/${form.client_id}?tab=coverage`); return; } const nextSection = targetSection(target); setSection(nextSection); if (nextSection !== "fields") return; window.setTimeout(() => { const container = document.querySelector<HTMLElement>(`[data-claim-field="${target}"]`); const control = container?.querySelector<HTMLElement>("input, select, textarea, button"); container?.scrollIntoView({ behavior: "smooth", block: "center" }); control?.focus(); }, 0); }
