...
function formFrom(record?: Record<string, unknown>): DrawerForm { return { client_id: text(record?.client_id, ""), payer_id: text(record?.payer_id, ""), rendering_provider_id: text(record?.rendering_provider_id, ""), patient_control_number: text(record?.patient_control_number, ""), payer_claim_number: text(record?.payer_claim_number, ""), service_date_from: text(record?.service_date_from, "").slice(0, 10), service_date_to: text(record?.service_date_to, "").slice(0, 10), total_charge_cents: Number(record?.total_charge_cents ?? 0) }; }
