...
function payerChecks(input: BillingReadinessInput): ReadinessCheck[] {
  ...
  if (!["active", "eligible"].includes(String(input.eligibilityStatus ?? ""))) {
    checks.push(result(
      ...
      "fail",
      true,
      `Eligibility is ${String(input.eligibilityStatus ?? "not verified").replaceAll("_", " ")}.`,
      "Verify active coverage for the service date.",
    ));
  } else {
    checks.push(result(
      "eligibility_active",
  ...
  if (input.authorizationRequired) {
    if (input.authorizationStatus !== "approved") {
      checks.push(result(
        ...
        "fail",
        true,
        `Authorization is ${String(input.authorizationStatus ?? "missing").replaceAll("_", " ")}.`,
        "Resolve the authorization before billing.",
      ));
    } else if (
      ...
      checks.push(result(
        ...
        "fail",
        true,
        "Authorization has no remaining units.",
        "Obtain additional authorized units.",
      ));
    } else {
      checks.push(result(
        ...
        "pass",
        false,
        "Authorization requirement is satisfied.",
      ));
    }
  } else {
    checks.push(result(
  ...
  if (input.providerEnrollmentStatus === "needs_revalidation") {
    ...
    checks.push(result(
      ...
      "fail",
      true,
      `Provider enrollment is ${String(input.providerEnrollmentStatus ?? "not confirmed").replaceAll("_", " ")}.`,
      "Resolve provider-payer enrollment before claim creation.",
    ));
  } else {
    checks.push(result(
      ...
      "pass",
      false,
      "Provider enrollment is approved.",
    ));
  }
...
export function evaluateBillingReadiness(input: BillingReadinessInput): BillingReadiness {
  ...
  if (!input.note || !["signed", "locked"].includes(String(input.note.note_status))) {
    checks.push(result(
      ...
      "fail",
      true,
      "The clinical note is not signed.",
      "Complete and sign the encounter note.",
    ));
  } else {
    checks.push(result(
      "note_signed",
      ...
      "pass",
      false,
      "Clinical documentation is signed.",
    ));
  }
  ...
  if (!input.diagnoses.length) {
    checks.push(result(
      ...
      "fail",
      true,
      "No encounter diagnosis is available for billing.",
    ...
    checks.push(result(
      ...
      "pass",
      false,
      "Encounter diagnosis is available.",
    ));
  }
  ...
  if (!input.serviceLines.length) {
    checks.push(result(
      ...
      "fail",
      true,
      "No billable service line is documented.",
      "Add a CPT/HCPCS service line.",
    ));
  } else {
    ...
    checks.push(
      invalid.length
        ? result(
            "service_line_incomplete",
            "Service Line",
            "fail",
            true,
            `${invalid.length} service line(s) are missing CPT/HCPCS, units, charge, or place of service.`,
            "Correct the service line before billing.",
          )
        : result(
            "service_line_complete",
            "Service Line",
            "pass",
            false,
            "Billable service line data is complete.",
          ),
    );
  }
