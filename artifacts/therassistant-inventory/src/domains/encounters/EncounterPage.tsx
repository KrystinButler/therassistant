...
export function EncounterPage() {
  ...
  async function sign() {
    ...
    try {
      ...
      setMessage("Clinical note signed and locked. Any unresolved billing requirements were routed to billing readiness.");
      await load();
    } catch (err) {
    ...
    <>
      ...
      <div className="thera-detail-grid">
        ...
        <section className="thera-card">
          ...
          <div style={{ display: "grid", gap: 8, marginTop: 12 }}><input className="thera-input" placeholder="ICD-10 code" value={diagnosisCode} onChange={(e) => setDiagnosisCode(e.target.value)} /><input className="thera-input" placeholder="Description" value={diagnosisDescription} onChange={(e) => setDiagnosisDescription(e.target.value)} /><button type="button" className="thera-action secondary" disabled={saving || !diagnosisCode.trim()} onClick={() => void addDiagnosis()}>+ Add Diagnosis</button></div>
        </section>
        ...
        <section className="thera-card thera-span-2">
          ...
          {signed ? <div className="thera-alert">Documentation is signed and locked. Billing or coding items may still be completed without changing the signed clinical note.</div> : <><label><div className="thera-field-label">Signature</div><input className="thera-input" value={signatureText} onChange={(e) => setSignatureText(e.target.value)} placeholder="Provider signature" /></label><div style={{ marginTop: 12 }}><button type="button" className="thera-action" disabled={saving || !noteText.trim() || !signatureText.trim()} onClick={() => void sign()}>{saving ? "Signing..." : "Sign & Lock Note"}</button></div></>}
        </section>
