...
export function PreSessionPage() {
  ...
  async function beginEncounter() {
    ...
    setError(null);
    try {
      const result = await startEncounter(appointmentId);
      if (!result.ok) {
    ...
    <>
      ...
      <div className="thera-detail-grid">
        <section className="thera-card">
          ...
          <div className="thera-definition-grid"><Field label="Patient" value={appointment.clientName} /><Field label="Provider" value={appointment.providerName} /><Field label="Service" value={`${appointment.serviceType} ${appointment.cptCode ? `(${appointment.cptCode})` : ""}`} /><Field label="Location" value={appointment.locationType.replaceAll("_", " ")} /><Field label="Registration" value={<StatusBadge value={appointment.registrationStatus} />} /></div>
