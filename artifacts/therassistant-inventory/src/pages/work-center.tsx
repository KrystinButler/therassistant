...
export function WorkCenterPage() {
  ...
  const queues = useMemo(() => [...new Set(items.map((item) => String(item.workqueue_type)))].sort(), [items]);
  const payers = useMemo(() => [...new Set(items.map((item) => String(item.payerName)).filter((name) => name && name !== "—"))].sort(), [items]);
    ...
    <>
      ...
      <section className="thera-card" style={{ marginBottom: 16 }}>
        <div className="thera-filter-row">
          ...
          <select className="thera-select" value={queue} onChange={(event) => setQueue(event.target.value)}><option value="all">All queues</option>{queues.map((value) => <option key={value} value={value}>{value.replaceAll("_", " ")}</option>)}</select>
          <select className="thera-select" value={priority} onChange={(event) => setPriority(event.target.value)}><option value="all">All priorities</option><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select>
          <select className="thera-select" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All statuses</option><option value="open">Open</option><option value="in_progress">In Progress</option><option value="pending">Pending</option><option value="snoozed">Snoozed</option><option value="reopened">Reopened</option><option value="completed">Completed</option><option value="cancelled">Cancelled</option></select>
          <select className="thera-select" value={payer} onChange={(event) => setPayer(event.target.value)}><option value="all">All payers</option>{payers.map((value) => <option key={value} value={value}>{value}</option>)}</select>
          <select className="thera-select" value={due} onChange={(event) => setDue(event.target.value as DueFilter)}><option value="all">All due dates</option><option value="overdue">Overdue</option><option value="next7">Next 7 days</option><option value="none">No due date</option></select>
        </div>
      </section>
        ...
        {!loading && filtered.length > 0 && (
          <div className="thera-table-wrap">
            <table className="thera-table">
              <thead><tr><th>Priority</th><th>Queue / Work</th><th>Related Record</th><th>Patient / Provider</th><th>Payer</th><th>Status</th><th>Due</th><th>Actions</th></tr></thead>
              <tbody>
                {filtered.map((item) => {
                  ...
                  return <tr key={item.id}>
                    ...
                    <td>
                      <div className="thera-filter-row">
                        ...
                        {!active && <button type="button" className="thera-action" disabled={savingId === item.id} onClick={() => { const note = promptNote("Why is this work being reopened?"); if (note) void execute(item.id, () => reopenWorkItem(item.id, note), "Work reopened."); }}>Reopen</button>}
                        <Link className="thera-action secondary" href={String(item.sourceRoute)}>Open Source</Link>
                      </div>
                      <details style={{ marginTop: 8 }}><summary className="thera-link">History ({item.history.length})</summary><div className="thera-stack" style={{ marginTop: 8 }}>{item.history.length ? item.history.map((history) => <div className="thera-work-card" key={history.id}><div className="thera-work-card-top"><span>{dateTime(String(history.created_at ?? ""))}</span><span>{history.old_status ? String(history.old_status).replaceAll("_", " ") : "—"} → {history.new_status ? String(history.new_status).replaceAll("_", " ") : "—"}</span></div>{history.old_priority !== history.new_priority && <div className="thera-muted">Priority: {String(history.old_priority || "—")} → {String(history.new_priority || "—")}</div>}<div>{String(history.note || "")}</div></div>) : <div className="thera-muted">No history recorded yet.</div>}</div></details>
                    </td>
                  </tr>;
                })}
              </tbody>
